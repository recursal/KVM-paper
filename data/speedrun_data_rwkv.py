...
parser = argparse.ArgumentParser(description="Speedrun dataset preprocessing")
parser.add_argument(
    "-d",
    "--dataset",
    type=str,
    default="JeanKaddour/minipile",
    help="Which HF dataset to convert",
)
parser.add_argument(
    "-n", "--split", type=str, default="train", help="Which split name to convert"
)
parser.add_argument("-l", "--dir", type=str, default=None, help="Dir to write to")
parser.add_argument(
...
if local_dir is None:
    local_dir = args.dataset[args.dataset.rindex("/") + 1 :]
...
eot = 0  # enc._special_tokens['<|endoftext|>'] # end of text token
...
def tokenize(doc):
    ...
    tokens = tokenizer.encode(doc["text"]) + [eot]
    ...
    for tokens in pool.imap(tokenize, fw, chunksize=16):
        ...
        if token_count + len(tokens) < args.shard_size:
            ...
            filename = os.path.join(
                DATA_CACHE_DIR, f"{local_dir}_{split}_{shard_index:06d}.bin"
            )
